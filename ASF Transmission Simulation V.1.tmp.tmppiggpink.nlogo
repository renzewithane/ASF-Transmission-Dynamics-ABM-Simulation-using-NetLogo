...
to setup
  clear-all
  set num-agents 100 ; You can change the number of agents as needed
  create-agents
  reset-ticks
   ask turtles
  [
    set shape "pigg" ; check out other under Tools > Turtle shape editor
    set size 2
    set color blue ; try differents color
  ]
end

to create-agents
  create-turtles num-agents [
    set color random 10 ; Set a random color for each agent
...
to move-agents
  ask turtles [
    rt random 30 - 15 ; Randomly turn between -15 and 15 degrees
    fd 1 ; Move forward by 1 unit
    set color pink  ; try differents color
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
